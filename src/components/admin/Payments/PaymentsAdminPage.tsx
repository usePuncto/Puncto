...
import { PaymentLinksListSection } from '@/components/admin/Payments/PaymentLinksListSection';
...
export default function PaymentsAdminPage() {
  ...
  const [paymentsTab, setPaymentsTab] = useState<'links' | 'boletos' | 'history'>('links');
  ...
  const [boletoListPage, setBoletoListPage] = useState(1);
  const [boletosPerPage, setBoletosPerPage] = useState(10);
  ...
  const [educationTab, setEducationTab] = useState<'tuitions' | 'links' | 'boletos' | 'history'>(
    'tuitions'
  );

  const isEducation = business?.industry === 'education';
  const onLinksTab = isEducation ? educationTab === 'links' : paymentsTab === 'links';
  const onBoletosTab = isEducation ? educationTab === 'boletos' : paymentsTab === 'boletos';
  const onHistoryTab = isEducation ? educationTab === 'history' : paymentsTab === 'history';
  ...
  const createPaymentLinkRequest = async (
    data: {
      name: string;
      description?: string;
      amount: number;
  ...
  const handleCreatePaymentLink = async (data: {
    ...
    await createPaymentLinkRequest(data, 'payment');
    setShowPaymentLinkForm(false);
  };

  const handleCreateBoletoLink = async (data: {
    name: string;
    description?: string;
    amount: number;
    currency: string;
    ...
    await createPaymentLinkRequest(data, 'boleto');
  ...
  const standardPaymentLinks = useMemo(
    () => (paymentLinks ?? []).filter((l) => l.linkKind !== 'boleto'),
    [paymentLinks]
  );
  const boletoPaymentLinks = useMemo(
    () => (paymentLinks ?? []).filter((l) => l.linkKind === 'boleto'),
    [paymentLinks]
  );

  const linkListTotal = standardPaymentLinks.length;
  const linkListTotalPages = Math.max(1, Math.ceil(linkListTotal / linksPerPage));
  const boletoListTotal = boletoPaymentLinks.length;
  const boletoListTotalPages = Math.max(1, Math.ceil(boletoListTotal / boletosPerPage));
  ...
  useEffect(() => {
    if (boletoListPage > boletoListTotalPages) {
      setBoletoListPage(boletoListTotalPages);
    }
  }, [boletoListPage, boletoListTotalPages, boletoListTotal]);
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-8 pb-10">
      ...
      <header className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        ...
        <div className="flex shrink-0 flex-wrap gap-2">
          <button
            type="button"
            onClick={() => {
              if (isEducation) setEducationTab('links');
              else setPaymentsTab('links');
              setShowBoletoForm(false);
          ...
          >
            {showPaymentLinkForm && (!isEducation ? paymentsTab === 'links' : educationTab === 'links')
              ? 'Fechar link'
          ...
          <button
            type="button"
            onClick={() => {
              if (isEducation) setEducationTab('boletos');
              else setPaymentsTab('boletos');
              setShowPaymentLinkForm(false);
              setShowBoletoForm((v) => !v);
            }}
            className="rounded-xl border border-neutral-300 bg-white px-4 py-2.5 text-sm font-medium text-neutral-900 hover:bg-neutral-50"
          >
            {showBoletoForm && (!isEducation ? paymentsTab === 'boletos' : educationTab === 'boletos')
              ? 'Fechar boleto'
              : 'Gerar boleto'}
          </button>
        </div>
          ...
          {(
            isEducation
              ? ([
                  ...
                  { id: 'boletos' as const, label: 'Boletos' },
            ...
            <button
              ...
              onClick={() => {
                if (isEducation) {
                  setEducationTab(tab.id);
                } else if (tab.id === 'links' || tab.id === 'boletos' || tab.id === 'history') {
              ...
              className={`rounded-t-lg px-4 py-2.5 text-sm font-medium transition-colors ${
                (isEducation ? educationTab : paymentsTab) === tab.id
      ...
      {onLinksTab && showPaymentLinkForm && (
        <section className="rounded-xl border border-neutral-200 bg-white p-6 shadow-sm">
          <h2 className="text-base font-semibold text-neutral-900">Criar link de pagamento</h2>
          <p className="mt-1 text-sm text-neutral-600">
            Gere um link para cartão ou Pix. O cliente paga na página segura da Stripe.
          </p>
          <div className="mt-5">
            <PaymentLinkForm
              businessId={business.id}
        ...
        <section className="rounded-xl border border-neutral-200 bg-white p-6 shadow-sm">
          ...
          <div className="mt-5">
            <PaymentLinkForm
              businessId={business.id}
              variant="boleto"
              onSubmit={handleCreateBoletoLink}
              onCancel={() => setShowBoletoForm(false)}
            />
          </div>
        </section>
      )}
      ...
      <PaymentLinksListSection
        ...
        description='URLs para cartão e Pix. Use "Copiar" para enviar por WhatsApp ou e-mail.'
        emptyMessage='Nenhum link ainda. Use "Gerar link de pagamento" acima.'
        links={standardPaymentLinks}
        loading={paymentLinksLoading}
        customerLabelById={customerLabelById}
        hasSucceededPaymentByLinkId={hasSucceededPaymentByLinkId}
        linkListPage={linkListPage}
        linksPerPage={linksPerPage}
        linkListTotal={linkListTotal}
        linkListTotalPages={linkListTotalPages}
        linkActionLoading={linkActionLoading}
        onSetLinksPerPage={setLinksPerPage}
        onSetLinkListPage={setLinkListPage}
        onLinkCustomer={(link) => {
          setLinkingCustomerFor(link);
          setLinkCustomerDraft(link.linkedCustomerId || '');
        }}
        onCancelLink={handleCancelPaymentLink}
        onCopy={handleCopy}
        formatAmount={formatAmount}
        isLinkExpired={isLinkExpired}
        getExpiresAtDate={getExpiresAtDate}
      />
      </>
      )}
      ...
      <PaymentLinksListSection
        title="Boletos"
        description="Links exclusivos para boleto. Enquanto não compensar, a situação fica como aguardando."
        emptyMessage='Nenhum boleto gerado. Use "Gerar boleto" acima.'
        links={boletoPaymentLinks}
        loading={paymentLinksLoading}
        customerLabelById={customerLabelById}
        hasSucceededPaymentByLinkId={hasSucceededPaymentByLinkId}
        linkListPage={boletoListPage}
        linksPerPage={boletosPerPage}
        linkListTotal={boletoListTotal}
        linkListTotalPages={boletoListTotalPages}
        linkActionLoading={linkActionLoading}
        onSetLinksPerPage={setBoletosPerPage}
        onSetLinkListPage={setBoletoListPage}
        onLinkCustomer={(link) => {
          setLinkingCustomerFor(link);
          setLinkCustomerDraft(link.linkedCustomerId || '');
        }}
